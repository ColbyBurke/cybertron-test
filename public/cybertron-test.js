  ...
  ** If you have time at the end, try and figure out how to take the
  ...
  `;
    ...
    function level2() {
      ...
      const exercise3 = _ => {
        return reduce((x, y) => y.eyeColor === 'green' ? x + 1 : x, 0, clients)
      };

      const ex4 = `Use map, filter and reduce with compose to return the full name (as a string) of the female from Wyoming. `;
      const exercise4 = _ => {
        return (
         compose(map(x => `${x.name.firstName} ${x.name.lastName}`), filter(x => x.address.state === 'Wyoming'), reduce((x, y) => {
           if(y.gender === 'female'){
             x.push(y);
             return x
           }
      ...
      const exercise5 = _ => {
        return compose(map(x => `${x.address.street} ${x.address.city}, ${x.address.state} ${x.address.zip}`),filter(x => x.address.state === 'North Carolina'))(clients)
      };

      const ex6 = 'use filter to remove anyone over the age of 25';
      const exercise6 = _ => {
        return filter(x => x.age < 26, clients)
      };

      const ex7 =
        'use reduce to count the number of males, age 22 - 27, who have green eyes';
      const exercise7 = _ => {
        return reduce((x, y) => y.gender === 'male' && y.age >= 22 && y.age <= 27 && y.eyeColor === 'green' ? x+1 : x ,0, clients)
      };
